...
- Live Firebase Hosting is `964d7c266db0dd2d`, the frontend-only Mutual-default/layout release cloned from `mutual-ui-20260910-0120` at **2026-09-10 01:22:50.239 UTC**. Backend, ledger, uploads and production rewrites are unchanged. [Release evidence](evidence/mutual-ui-release-2026-09-10.json) records 44 local browser workflows, strict hardening, 48 exact preview/live shell hashes, readiness, anonymous isolation, cold-offline launch, and the real old-to-new Update App transition. Authenticated hosted Report/evidence checks remain the September 7 results; they were not rerun with production accounts for this UI release. Physical-phone testing remains pending.
- The new preview expires **2026-09-17 01:20:57 UTC**. For frontend-only rollback, reverify that retained channel `report-release-20260907` still contains `f27b6a46dae98c71` (recorded expiry **2026-09-14 04:17:47 UTC**), then clone that exact channel to live without touching Cloud Run or the database. Never use the older SQLite demo channel for live rollback.
