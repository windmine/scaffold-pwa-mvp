...
- During the invited-account pilot, require the tested login shell to show `Invited accounts only` and keep public registration hidden. The local invitation release requires `0021_worker_invitations` and its backend before frontend promotion. Existing accounts default to completed setup and retain their passwords. New report-only Staff Workers use private, expiring setup links and cannot log in beforehand. Verify preview/local shell parity and the intended-recipient private-handoff procedure; do not claim automatic email delivery.
...
The database test specifically proves `pool_pre_ping` recovers a poisoned returned connection. The focused tests cover upload adapter parity, immutable Report Template snapshots/server-derived formulas, Report ownership/workflow/transition concurrency, cursor-paginated Review Queue policy/query/export separation, and migrations. `check:review-queue` also verifies Find-aware CSV/HTML/PDF exports across all matching pages; `check:invitations` checks private-link creation, acceptance, invalidation, scoped access, and concurrent/expired claims against an owned disposable SQLite backend. Stage PostgreSQL invitation contention separately before the `0021` release; these checks neither deploy the update nor send invitation email.
...
The 2026-09-15 combined local run passed all 66 app browser scenarios, 11 installer checks, the focused offline/editor checks, `check:review-queue`, backend suites, and an owned disposable-backend smoke test. Final UI/offline-publication refinements then passed lint/build/PWA/static checks, seven Template-draft UI scenarios with 320/390/1280px English/Chinese layouts, and the production-preview cold offline Report-return check on `leader-field-9a5e069c0e84`. The entire 66-scenario suite was not repeated after those refinements. The candidate remains unreleased; these checks do not replace its PostgreSQL/provider staging, hosted, or physical-phone gates.
...
The recorded production migration head is `0020_missing_snapshot_daywork_correction`; the explicit 2026-09-07 migration job applied `0018`–`0020` after draining production requests. Local source also contains the unreleased `0021_worker_invitations`; offline Template snapshots, private editor drafts, and the Template-edit concurrency guard require no further migration:
...
For the current local candidate, complete the existing `0021_worker_invitations` migration gate and promote the invitation, Find-aware export, and `expected_definition_version` Template-edit backend before its frontend. Offline snapshots and private editor drafts add no database migration. Do not treat earlier release evidence or focused local checks as approval for this combined update.
...
- The recorded coupled production release, post-migration recovery, preview/live parity, live authenticated Report workflow, and real waiting-service-worker update have green automated results. The **physical-phone gate remains pending**: verify both Worker classes, Supervisor navigation, optional Site, all Report states, exactly-once offline photo/signature replay, authenticated GCS streaming, the installed version's offline limits, translations, and updates. The local candidate's downloaded-Template cold returns and private editor drafts need their own matching staging/hosted/device verification; earlier live results do not cover them.
- The live release still uses initial passwords. Stage and release the local `0021` invitation update, verify the private recipient-handoff procedure on real devices, and complete the coupled migration/backend/frontend gate before using it beyond controlled accounts.
