...
Report PDFs derive 200-DPI page-sized rasters without rewriting uploaded evidence or department logos. A local 50-photo/6-megapixel export stress test checks all captions, embedded-image order, source hashes and page numbers. Before promotion, use owned staging fixtures to verify 50-photo submission/replay, authenticated original-image retrieval, single/collection PDFs, and exact frontend parity. Preserve existing production presentation fixtures. The frontend still stores original base64 images in IndexedDB and rewrites the record at checkpoints; physical-phone memory/storage and maximum-size 50 x 5 MB batches remain unverified. A stopped/killed app cannot continue an upload in the background. After any 50-photo Reports have been accepted, prefer keeping this compatible backend during a frontend rollback; do not assume the older eight-photo backend is a safe replay rollback.

The local 50-photo storage smoke passed with 64.56 MiB of originals, reload/partial replay and unchanged SHA-256 hashes, but sampled desktop Chromium memory reached 1.20 GiB. Treat this as a release risk requiring representative physical-phone validation, not proof that 50 maximum-size originals are safe. A per-image Blob store and smaller checkpoints are the next mitigation if those devices cannot handle the batch; do not silently lower original evidence quality.
