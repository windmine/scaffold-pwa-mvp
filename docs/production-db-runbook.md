...
### Local 2026-09-25 onboarding change - not deployed

Private manual handoff remains the operator's choice. No SMTP provider, sender, secret, production user or database migration was added. The setup frontend uses the distinct `POST /auth/login/after-setup` route for ordinary password authentication after invitation acceptance. This route always refuses any `__session`, legacy `geo_access_token` or Authorization header, even invalid/blank credentials, with HTTP 409 and no cookie mutation. Standard login remains compatible; invitation inspect/accept still omit browser credentials and acceptance never sets cookies. A cached local identity or unreadable storage also prevents frontend continuation. Do not substitute unguarded `/auth/login` as a continuation fallback. Old backends lacking the route fail safely to manual sign-in.
...
### 2026-09-18 production release - 50-photo Reports and compact review
...
Known reload UX limitation: application startup awaits queued replay before revealing the Worker screen. The unchanged server upload cooldown can therefore delay that screen; the hosted verifier allows a bounded 300-second replay wait. Do not interpret a waiting screen as permission to clear storage or force another submission. Keep the app open and preserve queued evidence/checkpoints; a non-blocking startup UI is follow-up work, not part of this release.
...
The following serving identities and September results describe the historical pre-cutover baseline. The September 18 release section above identifies the current backend/frontend pair. Do not infer current serving identities or rollback compatibility from this historical baseline.
