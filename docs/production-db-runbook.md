...
Backend topology and readiness rechecked on 2026-08-04:
...
- Hosted `/api/health/ready` reports database and GCS as healthy. Firebase Hosting version `6b499ef514142a09` matches the verified commit `b2dec22` build for `index.html`, `sw.js`, `offline.html`, and `manifest.webmanifest`, serves the invited-only login with public registration hidden and the login form before the install promotion, contains the cold-offline Worker shell, and keeps Staff and Work Form creation behind Add actions.
...
On 2026-08-10, the controlled-test gate passed with three warnings (incident-only Monitoring, six-hour Neon retention, and skipped budget verification). The strict gate remains red for real production use because no verified notification channel is attached.

Current warnings are operational decisions rather than hidden green checks:

- The automated 2026-08-10 frontend release pass is green and the cold-offline shell with scoped Site/attendance restoration is live. The remaining hosted phone pass must verify killed/refreshed offline launch, queued attendance replay, actual photo/signature streaming, and the waiting-service-worker update flow on a real installed device.
