...
### Local 50-photo Report candidate (not deployed)

The working source raises only Reports to 50 photos (5 MB each); signatures remain separate, and retained Daywork/Task Logs remain at eight. Deploy the compatible backend before promoting the frontend that permits 50; no migration or maintenance window is required by this change. The current live release below still limits Reports to eight. Do not claim this candidate is live based on a local build or Firebase preview.
...
Report PDFs derive 200-DPI page-sized rasters without rewriting uploaded evidence or department logos. A local 50-photo/6-megapixel export stress test checks all captions, embedded-image order, source hashes and page numbers. Before promotion, use owned staging fixtures to verify 50-photo submission/replay, authenticated original-image retrieval, single/collection PDFs, and exact frontend parity. Preserve existing production presentation fixtures. The frontend rewrites the record at checkpoints; physical-phone memory/storage and maximum-size 50 x 5 MB batches remain unverified. A stopped/killed app cannot continue an upload in the background. After any 50-photo Reports have been accepted, prefer keeping this compatible backend during a frontend rollback; do not assume the older eight-photo backend is a safe replay rollback.

The 2026-09-18 local frontend candidate stores new Report photos as original Blobs; no server database schema migration is needed. Release review found that already-open older tabs could replay Blob-only records from the old shared IndexedDB without their photos. New Report drafts/records therefore require isolated `scaffold-pwa-report-evidence-v1` storage, transparent new-client legacy reads, and mixed-version replay regression checks before promotion. Legacy base64 queues remain supported. Editable legacy drafts normalize in memory and write through the normal save only. UI previews own and revoke temporary object URLs. Full-record checkpoints remain, but immutable Blob references avoid repeatedly cloning large base64 strings. The earlier same-harness 64.56-MiB/50-photo comparison preserved every SHA-256 hash and reduced sampled Chromium working set from 1,385,697,280 to 469,479,424 bytes (about 66%). This mocked-transport result is not full-gallery/physical-phone/50 x 5 MB proof.

Frontend rollback caution: older frontends do not understand or display isolated `photoBlobs` drafts/queues. Do not roll back to such a frontend while devices have unfinished Blob-backed Reports; release a compatible forward fix or first confirm device drafts/queues are safely submitted. Never clear IndexedDB to make a rollback appear successful. Backend and hosted data formats are unchanged by this storage/UI update. Complete automated cold return, partial replay, scoped logout, mixed-version, updated-shell and original-image checks before promotion. On 2026-09-18 the operator explicitly approved live-pilot deployment while physical-phone large-camera-batch checks remain pending; record that exception, not a physical-phone pass.

### 2026-09-16 Report PDF-format production release
...
The following serving identities and September results describe the historical pre-cutover baseline. The September 15 release section above identifies the current backend/frontend pair. Do not infer current serving identities or rollback compatibility from this historical baseline.
