...
- The automated 2026-08-13 frontend release pass is green, and the cold-offline shell, scoped Site/attendance restoration, consistent consequential-action dialog, actionable Analytics exceptions, and 14px essential map/Analytics labels are live. The remaining hosted phone pass must verify killed/refreshed offline launch, queued attendance replay, actual photo/signature streaming, dialog behavior, outdoor label readability, and the waiting-service-worker update flow on a real installed device.
