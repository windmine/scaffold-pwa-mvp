...
Before the hosted pass, use disposable staging databases to verify startup rejects production `AUTO_MIGRATE=true`, missing/pending history, checksum drift, and unknown newer migrations before any storage probe or purge task. Verify `python -m app.migrations --check` is read-only and that readiness returns sanitized HTTP 503 after a staged ledger mismatch. Never alter the live migration ledger for this test. Apply migrations in a separate coordinated release step; older guarded images become unready when a newer migration is recorded. The startup fix adds no migration beyond `0019_report_daywork_purpose`.
