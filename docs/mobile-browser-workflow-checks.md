...
- The equivalent real-phone pass against the hosted Firebase URL is still pending; the 2026-08-10 release result below is an automated local-browser and hosted HTTP/API pass.
...
The invited-only shell, login-before-install ordering, list-first Staff/Work Form administration, and scoped cold-offline update are live in Firebase Hosting version `6b499ef514142a09`. Local, preview, and live shell hashes match. Complete the manual installed-phone cold-launch, replay, photo/signature, and update-flow steps below before treating the hosted device gate as complete.
