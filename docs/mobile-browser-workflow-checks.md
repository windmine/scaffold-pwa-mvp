...
- The equivalent real-phone pass against the hosted Firebase URL is still pending; the 2026-08-13 release result below is an automated local-browser and hosted HTTP/API pass.
...
The invited-only shell, login-before-install ordering, list-first Staff/Work Form administration, scoped cold-offline update, consistent consequential-action dialog, actionable Analytics exceptions, and 14px essential map/Analytics labels are live in Firebase Hosting version `c761984b7353028a`. Local, preview, and live shell hashes match. Complete the manual installed-phone cold-launch, replay, photo/signature, outdoor-readability, dialog, and update-flow steps below before treating the hosted device gate as complete.
