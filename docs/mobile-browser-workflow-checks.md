...
Firebase Hosting version `549d17a081d3e606` is the current ReportFlow report-only build. Its shell and generated worker match the September 10 verified preview and local artifact from `81a8820`; the prior report-only `964d7c266db0dd2d` remains the frontend rollback reference. Anonymous hosted branding, cold-offline and actual waiting-update checks pass for this build; authenticated Worker/Supervisor and real GCS evidence/replay remain the historical September 7 hosted results. Complete the report-only Worker, Supervisor, installed-PWA name/icon refresh, replay, evidence-streaming, translation, native date/keyboard, and update-flow steps below on a physical phone before treating the hosted device gate as complete. Do not uninstall or clear storage while unsynced Reports remain.
