...
### Installer and printed QR device checks

- Print the A4 installer PDF at its intended size, retain the QR's white margin, and scan the printed page with an actual Android and iPhone camera. Both must open the permanent `/install.html` URL, not a preview or an automatic download.
- On Android Chrome, use the offered install action or Chrome's menu and confirm installation. On iPhone Safari, use Share / Add to Home Screen / Add, keeping Open as Web App enabled when shown. Check the ReportFlow icon/name and that launching opens the main app rather than a separate installed guide.
- Verify manual instructions and the browser option when a QR opens an in-app browser, installation is unavailable, or the native prompt is cancelled. Check both installer languages, copying the URL and the PDF/PNG/SVG downloads.
- With an existing unsynced Report and a waiting update, open the installer in another tab. It must not activate the update, clear data or replay submissions. Return to ReportFlow to finish the Report and use its draft-safe Update App action. Do not uninstall or clear browser data while Reports remain unsynced.
- Once the updated shell is cached, close all tabs and reopen `/install.html` offline: the installation guide and QR must remain readable. Sign-in, new Template loading and first-time installation still require connectivity. This check does not replace the full Worker/Supervisor offline/evidence checklist.

The isolated `npm run check:install` gate exercises public-page behavior with browser-boundary simulations and a real generated worker; it does not establish these physical-device results.

## Latest Automated Pass

- 2026-09-10 **Installer and printable QR frontend release**: Hosting `654ff0044465966e` was cloned live from the exact verified `install-20260910-0528` preview at 05:34:34 UTC, using a frozen build of uncommitted installer changes on base `17ed1195b167d6b3d992a364032a4195f5557be0`. Backend/ledger/uploads/rewrites are unchanged. Lint/build, all 44 app workflows, 11 installer checks, 26 Department-default assertions, static PWA checks, Report/review checks, production dependency audit and strict hardening pass (existing six-hour Neon warning). Installer local coverage includes 320/390/1280px English/Chinese layouts with no overflow and 44px controls; install prompt outcomes are simulated browser boundaries. PNG, rasterized SVG and rendered A4 PDF independently decode to the permanent production `/install.html` URL. [Preview](evidence/hosted-install-preview-2026-09-10.json)/[live](evidence/hosted-install-live-2026-09-10.json) each passed 57 exact shell assets, five readiness probes, anonymous isolation and cold-offline app launch. Separate [preview installer](evidence/installer-preview-2026-09-10.json)/[live installer](evidence/installer-live-2026-09-10.json) checks verified all seven file hashes/MIME types, no-cache HTML, English/Chinese phone UI, and the exact cached installer with logo/QR after every prior page closed. All three live installer screenshots were visually reviewed. The real [waiting-update transition](evidence/hosted-install-waiting-update-2026-09-10.json) passed from the earlier ReportFlow shell to this release. Hosted checks were anonymous/read-only, without production accounts or native installation. Physical-phone printed scans, installation and the full installed-device checklist remain pending. [Release record](evidence/install-qr-release-2026-09-10.json).
- 2026-09-10 **ReportFlow branding frontend release**: source `81a8820` passed lint/build, 26 Department-default assertions, all 44 local Chromium workflows, static PWA checks, Report/review checks, clean production dependency audit and strict hardening (existing six-hour Neon warning). Hosting `549d17a081d3e606` was cloned live from the verified preview at 03:14:00 UTC; backend/ledger/uploads were unchanged. [Preview](evidence/hosted-reportflow-preview-2026-09-10.json) and [live](evidence/hosted-reportflow-live-2026-09-10.json) each passed 53 exact shell assets, five database/migration/GCS probes, anonymous isolation and cold-offline launch. Separate [preview](evidence/hosted-reportflow-branding-preview-2026-09-10.json)/[live branding](evidence/hosted-reportflow-branding-live-2026-09-10.json) checks verified the visible name/logo, manifest, four PNG dimensions, Apple metadata, preserved install identity and no 390px overflow. The [real waiting-update transition](evidence/hosted-reportflow-waiting-update-2026-09-10.json) passed from the earlier Mutual shell to ReportFlow with zero private cache paths/page errors. Hosted checks were anonymous/read-only; authenticated branding across roles/languages and layouts were tested locally. Physical-phone home-screen name/icon refresh and the full installed-device checklist remain pending. [Release record](evidence/reportflow-release-2026-09-10.json).
- 2026-09-10 **Mutual-default/layout frontend release**: all 44 local Chromium workflows passed, including Department-default boundaries and 320/390/768/1280px light/dark layouts with 44px controls, readable selected dates, and an unobscured Submit action. Hosting `964d7c266db0dd2d` was cloned live from the verified preview at 01:22:50 UTC; backend/ledger/uploads were unchanged. [Preview](evidence/hosted-mutual-preview-2026-09-10.json) and [live](evidence/hosted-mutual-live-2026-09-10.json) each passed 48 exact shell hashes, five database/migration/GCS probes, anonymous isolation and cold-offline launch. The [real waiting-update transition](evidence/hosted-mutual-waiting-update-2026-09-10.json) passed from the September 7 shell to the new Mutual login shell. These hosted tests were anonymous/read-only; authenticated defaults/layouts were tested locally. No physical-phone or new authenticated production workflow pass is claimed. [Release record](evidence/mutual-ui-release-2026-09-10.json).
...
Firebase Hosting version `654ff0044465966e` is the current ReportFlow report-only build with the public installer and printable QR. Its shell and generated worker match the September 10 `install-20260910-0528` preview and frozen local artifact containing uncommitted installer changes on base `17ed1195b167d6b3d992a364032a4195f5557be0`; the prior `549d17a081d3e606` on retained `reportflow-20260910-0312` is the frontend rollback reference, subject to exact-version/expiry reverification. Anonymous hosted installer/download, cold-offline app/guide and actual waiting-update checks pass for this build; authenticated Worker/Supervisor and real GCS evidence/replay remain the historical September 7 hosted results. Complete printed QR scans, native installation, and the report-only Worker, Supervisor, installed-PWA name/icon refresh, replay, evidence-streaming, translation, native date/keyboard, and update-flow steps below on a physical phone before treating the hosted device gate as complete. Do not uninstall or clear storage while unsynced Reports remain.
