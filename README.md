...
The local gate passed 69 browser workflows, 214 disposable-backend smoke checkpoints, PostgreSQL 17.6 and 18.4 at 64 checkpoints each, focused startup/onboarding/presentation checks, lint and build. Authenticated preview/live runs each passed 13 checkpoints with 50 synthetic photos and non-blocking replay; only owned Reports `16`/`17` were trashed and Templates `14`/`15` archived. Preview/live onboarding each passed five checks, with only owned invited Workers `17`–`20` resigned afterward. Live 64-asset shell/readiness/offline checks, seven installer assets and four waiting-update checks passed; installer/update screenshots were visually reviewed. Final infrastructure/log checks confirmed unchanged configuration, 100% traffic without tags and no ERROR-or-higher/5xx entries. These are automated browser results, not physical-phone or maximum-camera-batch certification.

The previous Blob/50-photo-compatible Hosting version `3e7a08c85e6ca5e5` is preserved in channel `rollback-20260925` until recorded **2026-10-01T23:03:40.749687450Z** expiry. Reverify that exact snapshot and backend compatibility before rollback. Never clear device storage or uninstall while unsynced Reports remain; prefer a compatible forward fix.

### Released - Easier Private Onboarding
...
The released `/auth/login/after-setup` endpoint refuses existing cookie/header credentials; old servers return an error and the UI falls back to normal sign-in. The compatible backend was deployed before the frontend. No database migration or email-provider configuration was added. Generation checks protect local state but cannot cancel a server-issued cookie response already in flight; independent simultaneous sign-ins are not globally atomic. Run `npm run check:onboarding`, `npm run check:invitations`, backend security/smoke and browser workflows; native sharing and physical phones still need manual checks.

### Released - Quieter Screens
...
### Released - Non-blocking Saved Submission Sync
...
## Previous Release - 50 Photos, Compact Review and PDF Downloads, 2026-09-18

This historical cutover is superseded by September 25 above. The compatible backend was promoted before Firebase Hosting version `3e7a08c85e6ca5e5` was cloned from the exact `release-20260918` preview at **2026-09-18 06:00:16.936 UTC**. The frozen frontend was `44e04c3`, production cache `leader-field-ed9e6cddf6aa`. Backend `geo-backend-report50-20260918` served 100% with no temporary tags, using source `d840fbf`, build `0220c539-b6e9-4026-83cd-37f02053df6e`, image `sha256:c9d87fd9afd9428767d65d719f990df6facabe31bdd6d6e2ecf1e1fda4609444`. No migration, maintenance pause, runtime configuration or infrastructure-policy change was required. The exact `0021` ledger and existing presentation records remain intact. See the [release record](docs/evidence/report-release-20260918/release.json) for final gate results, timing and exact fixture cleanup.

The [final local gate](docs/evidence/report-release-20260918/local-gates.json) passed all 68 app workflows in one complete run, 11 installer checks, mixed-version storage and submission-render race regressions, backend/export/PDF suites, disposable API smoke, dependency checks and strict hardening with only the existing six-hour recovery warning. The production-backend preview completed [12 hosted checkpoints](docs/evidence/report-release-20260918/preview-reports50-final/evidence.json), including 50 small synthetic photos plus a signature, interrupted upload/reload/resume, original-image retrieval, workflow and exact-owned cleanup. Existing demo data is retained. [Candidate/live PDF parity](docs/evidence/report-release-20260918/pdf-pixel-parity.json) covers all 18 presentation pages; hosted Chromium does not establish physical-phone or maximum-camera-batch readiness.

Post-promotion [live 50-photo verification](docs/evidence/report-release-20260918/live-reports50/evidence.json) passed all 12 checkpoints with zero browser page errors; its owned Report `15` was soft-deleted and Template `13` archived, with no cleanup failures or uncertain submission outcome. Live [64-asset shell checks](docs/evidence/report-release-20260918/live-shell.json), [installer checks](docs/evidence/report-release-20260918/live-installer.json) and [four real waiting-update checks](docs/evidence/report-release-20260918/live-waiting-update.json) also passed. These are hosted automated results, not physical-phone results.

The released UI adds compact **My Reports** cards with date, optional Site, status, photo count and a final-note/error preview. **Show details** opens the full answers/evidence; closing releases its local image URLs. Search still includes the full answers. On phones/tablets below 1100px, Supervisor filters collapse with their active context visible, selecting a Report opens a dedicated detail view, and **Back to Reports** restores the inbox. Review actions precede long evidence; Previous/Next stays in the detail view. Desktop retains side-by-side review.

PDF is the default single-Report download and the first, primary collection-export action. HTML and CSV remain available; retained Daywork export defaults are unchanged.

The released app accepts up to **50 photos per Report**, selected together or added in several batches; JPEG, PNG and WebP still have a **5 MB per-file** limit. Required handwritten signatures are additional to those 50 photos. Retained Daywork and Task Logs stay at eight photos. Devices still controlled by the previous service worker need the draft-safe **Update App** flow to receive the new interface.

New Report photos retain original File/Blob bytes in IndexedDB instead of base64 strings. Temporary preview URLs are not persisted and are released on replacement, collapse or logout. Existing base64 drafts/queues remain readable; editable legacy drafts convert in memory and are replaced only after a successful save. Later thumbnails load lazily, and submission shows upload progress. An explicit upload-rate-limit response can wait and retry at the server's requested delay (at most 60 seconds per wait, three retries per image and 120 seconds cumulative waiting per attempt). Other failures leave the Report queued with completed uploads checkpointed for later replay. Keep the app open during upload. A worst-case 50 x 5 MB selection still needs substantial storage and has not been validated on physical phones. A failed local save is not a durable draft; do not close the page until saving succeeds.

`npm run check:report-photo-memory` exercises 50 distinct synthetic PNGs (64.56 MiB), real IndexedDB, two cold reloads and partial-upload resume with byte-exact SHA-256 checks. Set `PHOTO_EVIDENCE_MODE=legacy` for the same base64 fixture. In the local same-harness comparison, sampled Chromium working-set peak fell from 1,385,697,280 to 469,479,424 bytes (about 66%); sampled JS heap fell about 80%. This is a mocked-transport storage/replay benchmark, not a full-resolution gallery, physical-phone or maximum-size certification. See the [release checklist](docs/mobile-browser-workflow-checks.md).

New Report drafts/records use the separate `scaffold-pwa-report-evidence-v1` IndexedDB namespace so already-open older tabs cannot replay them without their Blob photos. The current client reads both stores; legacy base64 queues remain in their original location. Legacy draft cleanup is conditional on the exact previously read contents, preserving concurrent edits. `check:report-photos` includes mixed-version tests against the deployed September 15 client. Old clients cannot display the new isolated drafts; keep a Blob-compatible frontend and never clear device storage while work is unfinished.

PDF exports use page-sized copies of large photos and signatures (200 DPI), keeping evidence order, proportions and original uploaded bytes unchanged. `npm run check:report-photos` checks the 50/51-photo backend boundary and bounded upload/replay behavior; `check:mobile` also exercises one-picker 50-photo selection, draft reload and throttled submission. `check:report-pdf` includes 50 distinct 6-megapixel photos, source-integrity and pagination checks. These local volume gates are distinct from hosted small-fixture checks and are not physical-phone approval.

The earlier base64 storage smoke reached 1.20 GiB sampled desktop memory; the Blob comparison above supersedes that implementation. The final isolated-storage rerun preserved every original across two reloads and partial replay, with a sampled 482,914,304-byte working-set peak. Physical-phone testing of realistic camera batches and 50 near-5-MB files remains pending. On 2026-09-18 the operator explicitly approved deployment to the controlled live pilot with this limitation; this is not broad phone-capacity approval. Details are in the [mobile checklist](docs/mobile-browser-workflow-checks.md#released-50-photo-reports-and-compact-review---2026-09-18).

Historical reload limitation: the September 18 frontend awaited queued replay before revealing the Worker screen. September 25 fixes that delay with a non-blocking screen and clear progress while preserving original evidence/checkpoints. Keep the app open until uploads finish. Prefer a compatible forward fix over rollback. Pre-Blob frontends cannot display isolated drafts/queues, and an eight-photo backend is not a safe default after 50-photo submissions are accepted. Never clear device data or uninstall to make a rollback appear successful.

## Previous Backend Release - Department-branded Report PDFs, 2026-09-16

Historically, backend `geo-backend-pdf-20260916-0010` was promoted at 100% with no temporary tags from committed source `7ad0f88`; the September 18 release above supersedes it. This PDF-only release introduced the supplied A4 form layout and department branding, including the supplied Mutual logo, without migration, maintenance or Hosting promotion. At that cutover the September 15 frontend and runtime configuration remained unchanged. [Release evidence](docs/evidence/report-pdf-release-20260916/release.json) preserves its checks; it is not a current rollback recommendation for 50-photo Reports. Physical-phone PDF-viewer checks remain pending.

## Previous Coupled Release - Reports, Drafts and Worker Invitations, 2026-09-15

The following records the historical September 15 cutover, not the current serving pair. Subsequent September 18 and September 25 releases supersede both its backend and frontend.

Hosting version `23c1ee5e712da7d7` is live, cloned from exact verified preview `release-20260915` at **2026-09-15 01:23:23.272 UTC**. Backend `geo-backend-report-20260915-0025` is healthy at 100% traffic with temporary tags removed. The approved database-free maintenance/drain window on `geo-backend-maint-20260915-0025` ran from **01:10:22.871295 UTC** until backend promotion at **01:17:26.168054 UTC**, about **7 minutes 3 seconds**. Migration `0021` and its separate read-only check passed; the [post-migration inventory](docs/evidence/production-postmigration-20260915.json) preserves the existing Report's immutable-content hash. The [final Current recovery proof](docs/evidence/neon-recovery-proof-20260915-current-final.json) and strict hardening gate passed, retaining only the six-hour recovery-window warning.

The frozen frontend is `921be2d`, production cache `leader-field-57d5f43f282c`; `750298f` changes only the hosted test runner. Backend image source is `8e33d21`. Staging passed the [10-check baseline](docs/evidence/hosted-report-stage-20260915-h/evidence.json), [seven-check improvements run](docs/evidence/hosted-improvements-stage-20260915-i/evidence.json), [63-asset shell/PWA pass](docs/evidence/hosted-stage-final-shell-20260915.json), and [installer pass](docs/evidence/hosted-stage-final-installer-20260915.json). Production passed [all 63 shell assets, readiness, anonymous isolation and cold-offline launch](docs/evidence/hosted-live-shell-20260915.json), the [installer](docs/evidence/hosted-live-installer-20260915.json), and the [four-check real waiting-update transition](docs/evidence/hosted-waiting-update-20260915.json) from `leader-field-615462fba4c3` to `leader-field-57d5f43f282c`; all six installer/update images were visually reviewed. The live [baseline](docs/evidence/hosted-report-live-20260915-a/evidence.json) and [improvements](docs/evidence/hosted-improvements-live-20260915-a/evidence.json) passed 10 and seven checks. Owned Templates `5`/`6` were archived, Reports `2`/`3` soft-deleted, invited Worker `12` resigned, and [dedicated accounts `9`/`10`/`11` deactivated](docs/evidence/hosted-accounts-live-20260915-a.json); existing accounts were unchanged. A 533-entry candidate-log review found no ERROR-or-higher or HTTP 5xx entries. See the [release record](docs/evidence/report-improvements-release-20260915.json) and [runbook](docs/production-db-runbook.md#2026-09-15-coupled-production-release). Physical-phone validation and real-recipient handoff remain pending; invitations are private/manual and send no automatic email.

## Previous Serving Release - Install Guide and Printable QR, 2026-09-10
...
Source commit `81a8820` was released as Hosting version `549d17a081d3e606` by cloning the exact verified preview at **2026-09-10 03:14:00 UTC**. Backend `geo-backend-report-release-202609070416`, migrations through `0020`, uploads and API rewrites are unchanged. Lint/build, all **44 local Chromium workflows**, 26 Department-default assertions, static PWA checks, Report/review checks, production dependency audit and strict hardening passed. [Preview](docs/evidence/hosted-reportflow-preview-2026-09-10.json) and [live](docs/evidence/hosted-reportflow-live-2026-09-10.json) each passed **53 exact shell assets**, five database/migration/GCS probes, anonymous isolation and cold-offline launch. Independent hosted branding checks and the real [Update App transition](docs/evidence/hosted-reportflow-waiting-update-2026-09-10.json) passed. These hosted checks were anonymous/read-only, not an authenticated production or physical-phone pass. [Release record](docs/evidence/reportflow-release-2026-09-10.json).

The [matching preview](https://geo-attendance-system-db9ca--reportflow-20260910-0312-5rs3ddps.web.app) expires **2026-09-17 03:11:53 UTC**. For frontend-only rollback, first reverify the retained `mutual-ui-20260910-0120` channel still holds the previous version `964d7c266db0dd2d`, then clone that channel without changing the compatible backend or database.
...
The earlier Mutual default branding/new-Staff selection and compact Report layouts were released as Firebase Hosting version `964d7c266db0dd2d`, cloned from the verified preview at **2026-09-10 01:22:50 UTC**. Existing account Departments and saved dashboard scopes were unchanged. Backend `geo-backend-report-release-202609070416`, migrations through `0020`, uploads, and Hosting API rewrites were unchanged. [Release record](docs/evidence/mutual-ui-release-2026-09-10.json).
...
The photo/draft, offline-return, private Supervisor Template-draft, Worker-invitation, and Find-aware export improvements below are deployed in the September 15 coupled release. The [final 2026-09-15 local rerun](docs/evidence/local-final-release-20260915.json) passed all **67 app workflows** and **11 installer checks** after the final Template action-lock fix, with lint, build and focused checks, at frontend `921be2d` / production cache `leader-field-57d5f43f282c`; backend image source remains `8e33d21`. Native [PostgreSQL 17.6](docs/evidence/postgres-rehearsal-17-2026-09-15-invitation-template-release.json) and [18.4](docs/evidence/postgres-rehearsal-18-2026-09-15-invitation-template-release.json) each passed 64 checkpoints with owned-cluster cleanup. The [provider-backed smoke test](docs/evidence/neon-provider-smoke-20260915.json) passed 209 checks on a separate owned Neon database through a loopback API; the [final hosted staging baseline](docs/evidence/hosted-report-stage-20260915-h/evidence.json) passed 10 Chromium/GCS Report checks, and the [hosted improvements run](docs/evidence/hosted-improvements-stage-20260915-i/evidence.json) passed seven checks with owned-fixture cleanup. Automated live verification passed as recorded above; physical-phone validation remains pending. Template-edit concurrency protection adds no migration; `0021_worker_invitations` and the compatible backend were released before the coupled frontend.
...
- A Report requires a Report Date, may omit Site, can include up to 50 photos plus configured handwritten signatures, and keeps an immutable Definition snapshot after submission.
...
- **Released PDF format (2026-09-16):** single and collection Report PDFs use a branded A4 form layout: blue section headings, grey label cells, bordered answers, inline handwritten signatures, photos and final review details. Each Report starts a new page and repeats its own department brand, submitter and saved Template title. Footers show the actual submitted Definition version, Report Date and collection-wide page numbers; no issue date is invented. Existing Leader/Mutual/MC/Stech/BOP artwork is used, while unknown departments keep their name with the neutral ReportFlow mark. This changes PDF presentation only, not saved Reports, CSV/HTML or retained Daywork exports.
...
- At this historical checkpoint, the installer release still used Supervisor-chosen initial passwords. The September 15 coupled release above supersedes that flow for new report-only Workers with private, single-use password-setup invitations.
...
Run the full hosted physical-phone checklist against the released report-only app: both Worker classes, Supervisor navigation, optional Site, all Report states, offline photo/signature replay exactly once, authorized evidence streaming, killed/refreshed cached-shell launch, and **Update App**. The automated hosted release and old-to-new service-worker checks are green, but are not a real-device pass. Before broader onboarding, verify private recipient handoff and the released Worker-invitation flow on physical phones; native PostgreSQL contention, migration `0021`, and the coupled release are complete. No invitation email is sent. Also replace owner-level Neon runtime access, load-test explicit pool/scale limits, and choose longer recovery or external logical backups. Keep the confirmed Monitoring destination and NZ$10 project budget evidence current; neither proves budget-email receipt or caps spending. Resolve remaining development-only npm advisories separately.
...
- Add up to 50 JPEG, PNG, or WebP photos of at most 5 MB each, in one or several selections. Use **Remove** beneath a thumbnail to discard only that photo; remaining photos keep their order after reload and submission. Required signatures must contain handwriting before submission and do not count toward the photo limit. Large real-camera batches and maximum-size selections remain subject to the physical-phone gate above.
...
New-Worker invitation endpoints (released with migration `0021`):
...
The Report Template editor sends its saved baseline as `expected_definition_version` with content updates. A stale baseline returns HTTP 409 with `detail.code=report_template_edit_version_conflict`, without changing the current Definition. Content writers share a row-lock/version check, including compatible legacy callers; status-only archive/reactivate behavior remains separate. The compatible backend was released before the updated client on September 15. This guard adds no migration; the invitation flow uses the deployed `0021` schema.
...
`npm run check:report-pdf` (also in `check:review-queue`) checks packaged department logos, A4 layout, snapshot/review metadata, per-Report continuation headers, global pagination, long-answer overflow, inline/repeated signatures, image aspect ratios, unavailable evidence and literal field markup. `python backend/report_pdf_preview.py` creates a synthetic two-page example under `output/pdf/` for visual review; it never reads the supplied reference or production data. Regenerate the committed backend PNGs from the app artwork and approved export logos with `npm run generate:report-export-logos` after changing those logos. Mutual uses the user-supplied blue/gold square artwork in `assets/icons/mutual-logo-export.png`, preserved byte-for-byte rather than the retained green SVG placeholder. They live under `backend/app/assets/report-logos`, so the existing Python Docker image includes them without runtime SVG dependencies. Render final PDFs for visual checks in addition to text-based tests.
...
The 2026-09-15 PostgreSQL **17.6 and 18.4** candidate runs each pass **64 checkpoints** through `0021`, including 31 measured contention races: 20 Report reviews, eight invitation claim/reissue/revoke/resign races, and three checked/legacy Template-edit races. Both owned clusters were removed. The invitation/resignation race exposed a stale audit flag; refreshing password-setup state after acquiring the invitation lock corrects that audit snapshot without changing the migration. Evidence: [PostgreSQL 17](docs/evidence/postgres-rehearsal-17-2026-09-15-invitation-template-release.json), [PostgreSQL 18](docs/evidence/postgres-rehearsal-18-2026-09-15-invitation-template-release.json). These native local checks are not provider-staging, deployment, or physical-phone approval.

The historical final 2026-09-07 PostgreSQL **17.6 and 18.4** runs each pass **53 checkpoints**, including the full `0020` chain, missing-snapshot correction/refusal/rollback cases, and 20 real concurrent-review races. Both owned clusters were removed. Historical evidence: [PostgreSQL 17](docs/evidence/postgres-rehearsal-17-2026-09-07-release.json), [PostgreSQL 18](docs/evidence/postgres-rehearsal-18-2026-09-07-release.json). The earlier failing [17](docs/evidence/postgres-rehearsal-17-2026-09-07-isolated.json)/[18](docs/evidence/postgres-rehearsal-18-2026-09-07-isolated.json) runs remain historical evidence of the unchanged `0019` defect. The correction is additive; no production ledger or submitted content was rewritten. These local passes do not replace candidate-image/provider staging or hosted phone checks.
...
For the new candidate, run `node scripts/check-hosted-report-improvements.mjs --allow-hosted-mutations --run-id <same-run-id>` after the baseline and before deactivating those three fixtures. It uses the same exact-host and account environment, requires exact provisioner emails and passwords of at least 32 characters, and optionally accepts a new `HOSTED_REPORT_IMPROVEMENTS_EVIDENCE_DIR`. Its seven checks cover private Template create/edit reload and stale 409 recovery, invitation replacement/password setup/single use, real-service-worker cold Report draft return/queue/replay, second-Worker privacy, and actual matching/empty Find CSV/PDF downloads. Python with the existing `pypdf` dependency parses downloads. It archives/soft-deletes/resigns only its separate nonce-owned fixtures, saves sanitized evidence, and never logs/screenshots invitation secrets. Four local guard/export-parser groups pass; the hosted command itself requires a separately recorded pass. It does not replace explicit revoke/account-switch regressions, baseline photo/signature/GCS checks, or installed-phone/update testing.
...
- Task Logs and retained Daywork support up to 8 progress photos. The released Report flow supports 50 photos plus configured handwritten signatures; physical-phone testing of maximum-size batches remains pending.
...
- Before onboarding beyond controlled pilot accounts, verify the released Worker-invitation flow on physical phones and confirm private intended-recipient handoff. Migration `0021` and its coupled backend/frontend are deployed and automated checks pass; no invitation email is sent automatically.
...
- The photo/draft, offline-return, Supervisor Template-draft, Worker-invitation and Find-aware export update is deployed with migration `0021` and the Template-edit version guard. Maintain the verified release/recovery evidence; offline return and editor drafts add no migration.
- Complete the physical-phone checklist against the September 15 report-only release above. Automated staging/live/update results are not an installed-device or real-recipient handoff pass.
