...
Known reload limitation: startup currently awaits queued replay before revealing the Worker screen. A server upload cooldown can therefore delay that screen while original evidence and upload checkpoints remain saved; keep the app open. Prefer a compatible forward fix over rollback. Older frontends cannot display isolated Blob drafts/queues, and an eight-photo backend is not a safe default after 50-photo submissions are accepted. Never clear device data or uninstall to make a rollback appear successful.
...
The following records the historical September 15 cutover, not the current serving pair. The September 18 release above supersedes both its backend and frontend.
