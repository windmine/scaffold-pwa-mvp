...
- During the invited-account pilot, public registration remains hidden. The local `0021_worker_invitations` update makes report-only Staff create pending Workers with private, expiring, single-use password-setup links; it sends no email and is not yet deployed. Existing accounts and retained/Supervisor provisioning remain compatible. Supervisors must verify the intended recipient and share links through an authenticated private channel. The verified-registration API remains callable and regression-tested, but is not exposed or supported as the pilot onboarding path. Deploy the invitation migration/backend before the coupled frontend.
...
- Local 2026-09-15 offline-return update: authenticated Worker loads download only active Report Template Definitions to exact Worker/Department-scoped IndexedDB snapshots. A saved identity, shell and Templates enable cold offline start/Continue/queue. Protected API responses, durable Report history and Supervisor lists remain network-only. Logout, explicit 401/403 and observed scope changes clear downloads; reconnect saves current answers/evidence before revalidation and retains existing Definition-conflict/replay guards.
- Local 2026-09-15 Supervisor Template editing update: private create/edit drafts save name, description, cards and exact unapplied raw syntax; Continue/Discard are visible and Close keeps work. Workspace navigation retains the live editor; Close/logout/update pause if saving fails. Stale, archived/unavailable or uncertain-publication copies remain read-only. Content edits send `expected_definition_version`; the backend rejects stale updates with HTTP 409 without a new migration. These changes are not deployed, and the prior invitation `0021` coupled-release requirement still applies.
- The 2026-09-15 combined local run passed all 66 app workflows, 11 installer checks, focused offline/editor checks, Review Queue/backend suites, and a disposable-backend smoke test. Final UI/offline-publication refinements then passed lint/build/static checks, seven Template-draft UI scenarios including English/Chinese 320/390/1280px layouts, and the cold offline Report-return check on `leader-field-9a5e069c0e84`; the full 66-scenario run was not repeated after those refinements. No staging/hosted/physical-phone pass is claimed. See `docs/mobile-browser-workflow-checks.md` for the current checkpoint.
...
2. Before broader onboarding, stage and release the completed local Worker-invitation flow: verify private recipient handoff and PostgreSQL contention, apply `0021_worker_invitations` and promote its compatible backend before the coupled frontend. The live release still uses Supervisor-chosen initial passwords. No invitation email is sent; automatic email delivery remains a separate transactional-provider integration.
