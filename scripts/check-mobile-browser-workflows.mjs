...
function parseGeneratedAppShell(workerSource) {
  const match = workerSource.match(/const APP_SHELL = (\[[\s\S]*?\]);/);
  return match ? JSON.parse(match[1]) : [];
}

function pwaShellCopies(publicPath) {
...
check('localized date inputs stay inside mobile form boundaries', () => (
  ...
  && sourceDateInputs.includes('input.showPicker?.()')
  && sourceStyles.includes('.date-input-shell > input[type="date"]')
  && sourceStyles.includes('.date-input-shell::after')
  && sourceStyles.includes('input[type="datetime-local"]::-webkit-calendar-picker-indicator')
...
check('PWA assets and cache name are generated from one manifest', () => {
  const expectedCacheLine = `const CACHE_VERSION = ${JSON.stringify(computePwaCacheVersion(root))};`;
  const distShell = parseGeneratedAppShell(distWorker);
  const productionEntrypoints = [...distIndex.matchAll(/<(?:script|link)\b[^>]*\b(?:src|href)="(\/[^"#?]+(?:[?#][^"]*)?)"/gi)]
    .map((match) => match[1].split(/[?#]/, 1)[0])
    .filter((publicPath) => /\.(?:css|js)$/i.test(publicPath));
  const expectedDistCacheLine = distShell.length
    ? `const CACHE_VERSION = ${JSON.stringify(computePwaCacheVersion(join(root, 'dist'), distShell))};`
    : '';
  const copiedShellPaths = pwaAssetCopies
    .map(([, target]) => target)
    .filter((target) => target !== 'sw.js')
    .map((target) => `/${target}`);

  return (
    sourceWorker.includes('Generated by scripts/generate-pwa-service-worker.mjs')
    && sourceWorker.includes(expectedCacheLine)
    && JSON.stringify(parseGeneratedAppShell(sourceWorker)) === JSON.stringify(appShell)
    && (!distWorker || (
      appShell.every((publicPath) => distShell.includes(publicPath))
      && productionEntrypoints.every((publicPath) => distShell.includes(publicPath))
      && distWorker.includes(expectedDistCacheLine)
    ))
    && copiedShellPaths.every((publicPath) => appShell.includes(publicPath))
    && pwaAssetCopies.every(([, target]) => hasFile(`dist/${target}`))
    && viteConfig.includes("from './scripts/pwa-shell-assets.mjs'")
    && viteConfig.includes('pwaAssetCopies')
...
check('production service worker uses the current app shell rules', () => (
  ...
  && distWorker.includes('ignoreVary: APP_SHELL.includes(requestPath)')
  && distWorker.includes("cache.match('/index.html')")
...
check('cold offline launch restores only Worker-scoped cached Site and attendance context', () => {
  ...
  return sourceApp.includes("from './offline-site-snapshot.js'")
    ...
    && sourceWorker.includes("cache.match('/offline.html')")
    && sourceWorker.includes('ignoreVary: APP_SHELL.includes(requestPath)');
});
...
check('browser auth uses HttpOnly cookie session with CSRF header', () => (
  sourceApiClient.includes('geo_csrf_token')
  && sourceApiClient.includes('X-CSRF-Token')
  && sourceApiClient.includes('clearLegacyBearerToken')
  && !sourceApiClient.includes('headers.Authorization')
  && read('backend/app/auth.py').includes('CSRF_COOKIE_NAME')
  && read('backend/app/main.py').includes('protect_cookie_authenticated_writes')
));
...
check('worker Log tab uses the Daywork work form path', () => (
  ...
  && sourceWorkerLog.includes('enhanceDayworkTeamMembers')
  && sourceWorkFormFields.includes('data-daywork-team-member-names')
  && sourceTeamMemberPicker.includes('data-team-member-search')
  ...
  && read('backend/app/main.py').includes('"id": "teams"')
  && read('backend/app/main.py').includes('"type": "time_range"')
  && read('backend/app/main.py').includes('"id": "team_break"')
  && read('backend/app/main.py').includes('"formula": "team_people * (team_time - team_break)"')
  && read('backend/app/use_cases/supervisor_review.py').includes('daywork_team_detail_rows')
));
...
check('normal workers are attendance-only and leaders can submit weekly team logs', () => (
  ...
  && sourceTeamMemberPicker.includes("type=\"checkbox\"")
  && sourceTeamMemberPicker.includes('data-team-member-search')
  && sourceTeamWorkLog.includes('rows.flatMap')
  && sourceTeamMemberPicker.includes('_selectedMemberIds')
  && sourceTeamWorkLog.includes('function timeOptions')
  && sourceTeamWorkLog.includes('function breakOptions')
  && sourceTeamWorkLog.includes('class="team-time-select"')
  && sourceTeamWorkLog.includes('No break')
  && sourceTeamWorkLog.includes('1 hour')
...
check('PDF exports are available for Daywork and submitted forms', () => (
  ...
  && sourceIndex.includes('value="form-submissions-csv"')
  && sourceIndex.includes('value="form-submissions-pdf"')
  && sourceIndex.includes('id="exportDateFrom"')
  && sourceIndex.includes('id="exportDateTo"')
  && sourceIndex.includes('id="exportFormTypeSelect"')
  && sourceIndex.includes('id="clearExportFiltersButton"')
  && sourceIndex.includes('class="export-action-grid"')
  && sourceStyles.includes('.export-document-grid')
  && sourceApiClient.includes('date_from')
  && sourceApiClient.includes('form_id')
  && sourceApiClient.includes('department_id')
  ...
  && read('assets/js/supervisor-review.js').includes('filters.departmentId = Number(state.departmentFocusId)')
  && read('assets/js/supervisor-review.js').includes("els.exportDocumentSelect.addEventListener('change', renderExportFormTypeOptions)")
  && read('assets/js/supervisor-review.js').includes('renderExportFormTypeOptions')
  && read('assets/js/supervisor-review.js').includes('runExport')
  && read('assets/js/supervisor-review.js').includes('clearExportFilters')
  && read('assets/js/history.js').includes('Daywork PDF')
  && read('backend/app/main.py').includes('/supervisor/form-submissions/export.csv')
  && read('backend/app/main.py').includes('/supervisor/form-submissions/export.pdf')
  && read('backend/app/use_cases/supervisor_review.py').includes('filter_records_by_department')
...
check('supervisor location map review is wired', () => (
  ...
  && pwaShellCopies('/assets/js/supervisor-map.js')
));
...
check('department supervisors cannot view or edit global admins', () => (
  read('backend/app/use_cases/staff_site_admin.py').includes(
    'Only global admins can edit global admin accounts'
  )
  && read('backend/app/use_cases/staff_site_admin.py').includes(
    'statement = statement.where(User.is_global_admin == False)'
  )
  ...
  && read('assets/js/staff-sites.js').includes('renderStaffCreateControls')
